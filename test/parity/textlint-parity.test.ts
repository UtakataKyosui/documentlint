 ...
 */
function mapTextlintSeverity(severity: TextlintMessage["severity"]): ComparableSeverity {
  switch (severity) {
    case 2:
      return "error";
    case 1:
    ...
    case 3:
      return "info";
    default:
      throw new Error(`Unexpected textlint severity in parity test: ${String(severity)}`);
  }
}

function expectedFromMessage(message: TextlintMessage, filePath: string): ComparableDiagnostic {
  return {
...
function readFixture(fixtureName: string): { readonly filePath: string; readonly text: string } {
...
describe("textlint adapter parity with textlint itself", () => {
  ...
  it.each(fixtureNames)("matches textlint's lint diagnostics for %s", async (fixtureName) => {
    const { filePath, text } = readFixture(fixtureName);

    const referenceResult: TextlintResult = await reference.lintText(text, filePath);
    const documentlintResult = await adapter.lintText(text, filePath);

    expect(referenceResult.messages.length).toBeGreaterThan(0);
    expect(documentlintResult.diagnostics.length).toBe(referenceResult.messages.length);
  ...
  it("matches textlint's fix output and applied/remaining diagnostics for fixable.md", async () => {
    ...
    const expectedApplied = sortDiagnostics(
      referenceFix.applyingMessages.map((message) => expectedFromMessage(message, filePath))
    );
    const actualApplied = sortDiagnostics(documentlintFix.applied.map(actualFromDiagnostic));
