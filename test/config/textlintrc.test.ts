...
describe("resolveTextlintrc", () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "textlint-rule-preset-foo", version: "1.0.0", main: "index.js" })
    );
  ...
  it("resolves rules, filters, and plugins from short names", () => {
    ...
    expect(result.filters[0]?.module.packageName).toBe("textlint-filter-rule-comments");

    expect(result.plugins).toHaveLength(1);
    expect(result.plugins[0]?.module.packageName).toBe("@textlint/textlint-plugin-markdown");
  });
  ...
  it("throws UnsupportedConfigError for an unknown top-level key", () => {
    ...
    expect(() => resolveTextlintrc(configPath)).toThrowError(UnsupportedConfigError);
    try {
  ...
  it.each([".textlintrc.yml", ".textlintrc.yaml", ".textlintrc.js", ".textlintrc.cjs"])(
    "throws UnsupportedConfigError for a non-JSON config file (%s)",
    (fileName) => {
      const nonJsonPath = path.join(tempDir, fileName);
      expect(() => resolveTextlintrc(nonJsonPath)).toThrowError(UnsupportedConfigError);
    }
  );
  ...
  it("propagates ModuleResolutionError for an uninstalled rule", () => {
    ...
    expect(() => resolveTextlintrc(configPath)).toThrowError(ModuleResolutionError);
  });
