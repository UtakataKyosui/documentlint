...
function createMessage(
	overrides: Partial<TextlintMessage> = {},
): TextlintMessage {
	return {
		type: "lint",
		ruleId: "example-rule",
...
describe("normalizeMessage", () => {
	it("maps severity 2 to error", () => {
		const diagnostic = normalizeMessage(
			createMessage({ severity: 2 }),
			"doc.md",
		);
	...
	it("throws on severity 0, which textlint uses for disabled rules", () => {
		expect(() =>
			normalizeMessage(createMessage({ severity: 0 }), "doc.md"),
		).toThrowError();
	});
...
describe("normalizeResult", () => {
	it("normalizes all messages and carries the filePath", () => {
		...
		expect(
			lintResult.diagnostics.every(
				(diagnostic) => diagnostic.filePath === "doc.md",
			),
		).toBe(true);
	});
});
...
describe("normalizeFixResult", () => {
	it("maps messages/applyingMessages/remainingMessages to diagnostics/applied/remaining", () => {
		const result: TextlintFixResult = {
			...
			applyingMessages: [
				createMessage({ ruleId: "rule-applied", severity: 1 }),
			],
			remainingMessages: [
				createMessage({ ruleId: "rule-remaining", severity: 3 }),
			],
		};
		...
		expect(
			fixResult.diagnostics.map((diagnostic) => diagnostic.ruleId),
		).toEqual(["rule-all"]);
		expect(fixResult.applied.map((diagnostic) => diagnostic.ruleId)).toEqual([
			"rule-applied",
		]);
		expect(fixResult.remaining.map((diagnostic) => diagnostic.ruleId)).toEqual([
			"rule-remaining",
		]);
	});
});
