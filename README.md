...
Run `documentlint [--version | --init | --print-config] [--debug] [--config .textlintrc.json] [--fix] [--dry-run] [--max-iterations n] [--format human|json] [--all | --git-staged | --git-changed | --git-since rev | --jj-revision rev | --jj-since rev | files/globs...]`.
...
Use `--version` without a configuration file to print the installed package version. `--init` creates a minimal `documentlint.json` and never overwrites an existing configuration. `--print-config` shows the selected configuration path and effective configuration. `--debug` writes configuration and target-selection decisions to stderr, leaving JSON stdout machine-readable.
