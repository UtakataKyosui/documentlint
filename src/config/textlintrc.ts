...
function resolveRuleSection(
  ...
  for (const [name, value] of Object.entries(section)) {
    ...
    if (value === true) {
      entries.push(buildEntry(name, resolveModule(kind, name, baseDir), undefined, undefined));
      continue;
    }

    if (isPlainObject(value)) {
      if (!isPreset) {
        entries.push(buildEntry(name, resolveModule(kind, name, baseDir), value, undefined));
        continue;
      }
...
export function resolveTextlintrc(configPath: string): ResolvedTextlintrc {
  ...
  return {
    ...
    filters: resolveSimpleSection(raw.filters, "filter", "filters", configPath, baseDir),
    plugins: resolveSimpleSection(raw.plugins, "plugin", "plugins", configPath, baseDir)
  };
}
