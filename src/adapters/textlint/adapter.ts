...
import { normalizeFixResult, normalizeResult } from "../../diagnostics/normalize.js";
...
function resolvePresetRuleOptions(
  ...
  const override = presetRules?.find((candidate) => candidate.ruleId === ruleKey);
...
async function buildRules(rc: ResolvedTextlintrc): Promise<TextlintKernelRule[]> {
  ...
  for (const entry of rc.rules) {
    ...
    const rule = await importDefault<TextlintRuleModule>(entry.module.resolvedPath);
    rules.push({
      ruleId: entry.ruleId,
      rule,
...
async function buildFilterRules(
  ...
  return Promise.all(
    filters.map(async (entry) => {
      const rule = await importDefault<TextlintFilterRuleReporter>(entry.module.resolvedPath);
      return {
        ruleId: entry.ruleId,
        rule,
        ...(entry.options !== undefined ? { options: entry.options } : {})
...
async function buildPlugins(
  ...
  return Promise.all(
    plugins.map(async (entry) => {
      const plugin = await importDefault<TextlintPluginCreator>(entry.module.resolvedPath);
      return {
...
async function buildDescriptor(rc: ResolvedTextlintrc): Promise<TextlintKernelDescriptor> {
...
export async function createTextlintAdapter(
  ...
  const linter = createLinter({
    descriptor,
    ...(options.ignoreFilePath !== undefined ? { ignoreFilePath: options.ignoreFilePath } : {}),
