...
import type { Diagnostic, DiagnosticSeverity, FixEdit, FixResult, LintResult, Suggestion } from "./types.js";

/** severity 0 は textlint では "none"（ルール無効）を意味し、報告対象の message には現れない。 */
function mapSeverity(severity: TextlintMessage["severity"]): DiagnosticSeverity {
  switch (severity) {
    case 2:
      return "error";
    case 1:
...
export function normalizeMessage(message: TextlintMessage, filePath: string): Diagnostic {
  const fix = message.fix !== undefined ? toFixEdit(message.fix) : undefined;
  const suggestions = message.suggestions !== undefined ? message.suggestions.map(toSuggestion) : undefined;
...
export function normalizeResult(result: TextlintResult): LintResult {
  return {
    ...
    diagnostics: result.messages.map((message) => normalizeMessage(message, result.filePath))
  };
}
...
export function normalizeFixResult(result: TextlintFixResult): FixResult {
  return {
    ...
    diagnostics: result.messages.map((message) => normalizeMessage(message, result.filePath)),
    applied: result.applyingMessages.map((message) => normalizeMessage(message, result.filePath)),
    remaining: result.remainingMessages.map((message) => normalizeMessage(message, result.filePath))
  };
}
